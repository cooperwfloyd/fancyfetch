...
const fancyfetch = async (resource, options, extras) => {
  ...
  if (
    extras?.maxTries &&
    (typeof extras.maxTries !== `number` ||
      !Number.isInteger(extras.maxTries) ||
      extras.maxTries < 1)
  )
    throw new Error(
      `Error in fancyfetch: extras.maxTries must be a positive integer.\n\nextras.maxTries: ${extras.maxTries}`
    );
  if (extras?.validateResponse && typeof extras.validateResponse !== `function`)
    throw new Error(
  ...
  for (let index = 0; index < (extras?.maxTries || 1); index++) {
    try {
      ...
      if (
        !!(
          ...
          !!extras.validateResponse(response, index)
        ) ||
        !!(!extras?.validateResponse && response)
      ) {
        result = response;
